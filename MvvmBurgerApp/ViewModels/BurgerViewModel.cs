...
namespace MvvmBurgerApp.ViewModels
{
    public class BurgerViewModel : INotifyPropertyChanged
    {
        ...
        public BurgerViewModel()
        {
            ...
            Burger.Cucumber = new Additionally("Огурец", 15, 0);
            Burger.Onion = new Additionally("Лук", 3, 0);
            //{
        ...
        private void RecalTotal()
        {
            if (Count > 0)
            {
                Total = Count * Price + SalatCount * Burger.Salat.Price + TomatCount * Burger.Tomat.Price + CucumberCount * Burger.Cucumber.Price + OnionCount * Burger.Onion.Price;
            }
        }
